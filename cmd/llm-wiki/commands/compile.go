...
func NewCompileCmd() *cobra.Command {
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for _, src := range sources {
				...
				for _, doc := range docs {
					if !force && !store.NeedsCompilation(src.Name, doc, promptHash) {
					...
					if err := store.StoreDocumentPages(src.Name, doc, pages, promptHash); err != nil {
	...
	cmd.Flags().BoolVar(&force, "force", false, "Force full recompilation (ignore cache)")
